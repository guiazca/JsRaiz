/*
  TODO:
  1 - Passar para template strings na listagem de usuários => OK
  2 - Substituir "for" por "map" => ok
  3 - Utilizar destructuring na extração das propriedades de cada usuário com let
  4 - Utilizar valor de fallback caso não tenha valor de telefone no usuário para exibir o texto 'Não cadastrado' => ok
  5 - Trocar if (status) por operador ternário => ok
  6 - Utilizar função auxiliar para tratar o status => ok
  7 - Utilizar arrow function na function do map =>
  8 - Utilizar destructuring de user para parâmetro da function do map => Ok
  9 - Utilizar retorno imediato ao invés da palavra reservada return no map => OK
